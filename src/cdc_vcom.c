...
// precheck before really using vcom_write to avoid some waiting time.
uint32_t vcom_write_precheck(void)
{
	VCOM_DATA_T *pVcom = &g_vCOM;
	return ( (pVcom->tx_flags & VCOM_TX_CONNECTED) && ((pVcom->tx_flags & VCOM_TX_BUSY) == 0) );
}

//int CDC_OutputHexValue(uint32_t value)
//{
//    char    temp_str[8+1];
//    int     byte_length, return_value;
//    byte_length = itoa_16(value,temp_str);
//
//    return_value = Chip_UART0_SendRB(LPC_USART0, &uart0_txring, (const uint8_t *) temp_str, byte_length);
...
int CDC_OutputHexValue_with_newline(uint32_t value)		// Assume that HEX is at most 8-digits
{
    char    temp_str[8+1+2];
    int     byte_length, return_value;
	...
	do
	{
		return_value =  vcom_write((uint8_t *)temp_str, (uint32_t) byte_length);
	}
...
int CDC_OutputString(char *str)
{
	...
	if (vcom_connected() != 0)
	{
		...
		if(temp_len!=0)
		{
			do
			{
				return_value =  vcom_write((uint8_t *)str, (uint32_t) temp_len);
			}
			while (return_value==0);
		}
	}
...
//int CDC_OutputData(uint8_t *str, int len)
//{
//	int return_value;
//    return_value = Chip_UART0_SendRB(LPC_USART0, &uart0_txring, (const uint8_t *) str, len);
//    return return_value;
//}
//
int CDC_OutputString_with_newline(char *str)
{
    int return_value = 0;
    return_value = CDC_OutputString(str);
#if (NEW_LINE_SYMBOL==_R_N_)
    return_value += CDC_OutputString("\n\r");
#else
    return_value += CDC_OutputString("\n");
#endif //  (NEW_LINE_SYMBOL==_R_N_)
