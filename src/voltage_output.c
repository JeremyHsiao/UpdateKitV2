 ...
 ****************************************************************************/
uint8_t				pwm_selection_before_off_command;
...
uint8_t	Get_PWM_Sel_Value(void)
{
	return current_duty_cycle_selection;
}
...
static inline uint8_t Change_PWM_Selection(bool decrease_dir)
{
	if(!decrease_dir)
	{
		if(++current_duty_cycle_selection>(MAX_DUTY_SELECTION_VALUE+DUTY_SELECTION_OFFSET_VALUE))
		{
			...
			pwm_selection_before_off_command = (MAX_DUTY_SELECTION_VALUE+DUTY_SELECTION_OFFSET_VALUE);
		}
	}
	else
	{
		...
		{
			current_duty_cycle_selection--;
			if(current_duty_cycle_selection==PWM_OFF_DUTY_SELECTION_VALUE)
			{
				pwm_selection_before_off_command=PWM_OFF_DUTY_SELECTION_VALUE+1;
			}
		}
	}
	return current_duty_cycle_selection;
...
uint8_t Set_PWM_Selection_by_Duty_Cycle(uint8_t new_duty_value)
{
	if(new_duty_value>(MAX_DUTY_SELECTION_VALUE))
	{
		new_duty_value=MAX_DUTY_SELECTION_VALUE;
	}
	current_duty_cycle_selection=(new_duty_value+DUTY_SELECTION_OFFSET_VALUE);

	return current_duty_cycle_selection;
}

uint8_t Set_PWM_Selection_Value(uint8_t new_value)
{
	if(new_value>(MAX_DUTY_SELECTION_VALUE+DUTY_SELECTION_OFFSET_VALUE))
	{
		new_value=(MAX_DUTY_SELECTION_VALUE+DUTY_SELECTION_OFFSET_VALUE);
	}
	current_duty_cycle_selection=new_value;
...
//			// only "entering user-ctrl mode" is allowed
...
//					EVENT_Button_pressed_debounced = false;	// discard button event if enter user-ctrl command has been received.
//				}
//			}
...
//			}
//			// process event generated by CommandExecution() immediately
//			if(EVENT_Leave_User_Ctrl_Mode)
//			{
//				EVENT_Leave_User_Ctrl_Mode = false;
//				// fall-back check protection -- both enter/leave ctrl-mode shouldn't be true at the same time
//				if(EVENT_Enter_User_Ctrl_Mode)
//				{
//					EVENT_Enter_User_Ctrl_Mode = false;
//					OutputString_with_newline("both enter/leave ctrl-mode are ON!"); // debug purpose
//				}
//				else
//				{
...
//				}
//			}
//			else if(EVENT_Enter_User_Ctrl_Mode)
//			{
//				EVENT_Enter_User_Ctrl_Mode = false;
//				return_next_state = US_PWM_USER_CTRL;
//			}
...
//			}
			if(EVENT_UART_CMD_Received)
			{
				...
				if(CommandInterpreter(command_string, &received_cmd_packet))
				{
					uint8_t	temp_pwm_sel = Get_PWM_Sel_Value();
					CommandExecution(received_cmd_packet,&ret_str);
					if(temp_pwm_sel!= Get_PWM_Sel_Value())
					{
						return_next_state = US_PWM_CHECK_SEL;
					}
				}
...
UPDATE_STATE System_State_Begin_Proc_for_voltage_output(UPDATE_STATE current_state)
{
	...
	switch(current_state)
	{
		...
		case US_PWM_USER_CTRL:
			...
			// Reset all events/variables generated by command execution
			EVENT_Leave_User_Ctrl_Mode = EVENT_Enter_User_Ctrl_Mode = false;
